...
if (empty($_POST["mail"]))
{
 ...
 if ($unErr ==""&& $passErr==""&&$fnameErr==""&&$lErr==""&&$BdErr==""&&$relerr==""&&$prerr==""&&$pererr==""&&$phoneerr==""&&$emailerr==""&&$linkerr=="") {
	...
	if (file_exists("../model/admindata.json")){
		...
		$pass = validate($pass);
		$handle = fopen("../model/userdata.json", "a");
		$arr1 = array('pass'=>$pass,'firstname' => $firstname, 'lastname' => $lastname,'gender'=>$gender,'u_birthday'=>$u_birthday,'Religion'=>$Religion,'Address'=>$u_address,'Permanent'=>$per_address,'Phone'=>$Phone,'urlink'=>$u_link,'mail'=>$mail,'username'=>$u_name);
...
<center><h1>User Entry Form</h1></center>
